...
class Graph:
    ...
    def calculate_AF_order_parameter(self, node, order_parameter, vertices, l_z):
        visited = [False] * (max(self.graph) + 1)
        sign = [0] * (max(self.graph) + 1)
        queue = [node]
        visited[node] = True

        if vertices[node][2] > l_z/2:
            sign[node] = 1
        else:
            sign[node] = -1
            ...
            for i in self.graph[node]:
                if not visited[i]:
                    ...
                    if vertices[i][2] > l_z / 2:
                        order_parameter += sign[i]
                    else:
                        order_parameter += -1 * sign[i]

        return order_parameter, visited
